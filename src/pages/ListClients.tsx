...
const ListClients: React.FC = () => {
  ...
  const getClients = async () => {
    try {
      ...
      // 🧩 Conversion automatique des IDs en nombre
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingClientId) {
        ...
        // ✅ Ajout avec ID numérique personnalisé
        const newClient = {
          id: Number(Date.now().toString().slice(-3)), // ex: 234
          nom,
          email,
          telephone,
        };
  ...
  const deleteClient = (id: number , nom: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`${API_URL}/clients/${Number(id)}`, {
            method: "DELETE",
          });
  ...
  // 🔹 Pré-remplissage du formulaire (modification)
  const editClient = (client: Client) => {
    setEditingClientId(Number(client.id)); // ✅ toujours un nombre
    setNom(client.nom);
